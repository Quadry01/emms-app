...
const MailDetails = () => {
  ...
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
   const options = ['RMO', 'Bursar']; // Static options
...
<div className=" text-center mb-4">
      <h3  className="mb-4 text-3xl text-center font-bold text-sky-900">
        Select Office</h3>
      <input
        type="text"
        value={inputValue}
        onChange={handleInputChange1}
        className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-lg  border-5 border border-gray-300 focus:ring-blue-500 focus:border-blue-500" // Basic styling
        ...
        <ul className="absolute z-10 bg-white text-gray-700 rounded shadow-lg mt-1 w-48">
          {options.map((option, index) => (
            <li
              ...
              className="block px-4 py-2 hover:bg-gray-100 cursor-pointer" // Add cursor-pointer
            >
              {option}
            </li>
          ))}
                  ...
                  ? item.current_location.split(",").map((location, index) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-center str">
                            {location.trim()}
                          </span>
                          <span className="text-sky-900 text-base font-normal text-center"></span>
                        </div>
                        {/* Divider Line */}
                        {index <
                          item.current_location.split(",").length - 1 && (
